...
- **Current Active Milestone:** Phase 3: Hosted — Permanent Deployment & Public App, **nearly complete**. The webhook server is deployed and live at `https://readme-realist.onrender.com` (Render, from `main`), the GitHub App webhook is repointed at it, the App is public, and **all three Milestone 6 live checks passed** end to end against it — first on 2026-08-28 (PR #12) and independently re-verified the same day (PRs #14 / #15). The stale default `gemini_model` is now **fixed** (`gemini-3.7-flash` → `gemini-2.5-flash`, the model the Render service already runs). Remaining: publish the install URL on the website (separate repo); and — as owner/follow-up items — move the `v1` alias forward to the `v2` commit (needs a tag force-push) and tick Publish-to-Marketplace on the `v2` release.
- **Last Updated:** 2026-08-28
- **Overall Status:** Core application complete and fully verified end-to-end live against GitHub. Gemini backend (`gemini-2.5-flash`) evaluated live PR diffs and successfully posted structured `NEEDS_UPDATE` verdict comments and updated Check Runs on GitHub (PRs #3, #14, #15). Offline suite re-verified 2026-08-26: **330/330 passing**. The repository is **public**; releases are **`v2` ("ReadMe Realist 2.0", latest)** and **`v1.0.0`**, with moving **`v1`** and **`v2`** major aliases. The GitHub Action has been live-verified end to end on a real runner, and the hosted App server now runs permanently on Render. Focus is distribution — see the Distribution Roadmap below.
...
- [x] **Observe live `NEEDS_UPDATE` PR comment** — 2026-08-28 on throwaway PR #12, re-verified on PR #14 against the Render deployment. An undocumented `diff_context_lines` (`DIFF_CONTEXT_LINES`) added to `Settings` drew a `readme-realist[bot]` drift comment with the exact README table row to add, plus a `neutral` "ReadMe Realist / documentation drift" Check Run titled "Documentation needs updating".
- [x] **Prove resolve path** — same PR #14: a follow-up commit adding the row to the README configuration reference rewrote the *same* comment (count stayed 1) to "✅ Documentation is up to date" ("re-reviewed `17ce84a`") and flipped the Check Run to `success` / "Documentation is up to date".
- [x] **Confirm whitespace-only push triggers zero-LLM noise skip path live** — 2026-08-28 on PR #12, re-verified on PR #15 (a one-blank-line diff): the Check Run resolved to `success` / "No documentation impact" with no PR comment and no LLM call (the local `_should_skip` noise-only branch).
...
- [x] Tag and release `v1.0.0`, then `v2` ("ReadMe Realist 2.0", latest) once the Phase 3 fix landed on `main`; `v1` and `v2` are moving major aliases. `README.md` and `examples/readme-realist.yml` pin `@v2`. **Follow-up (open):** move the `v1` alias forward to the `v2` commit — `git tag -f v1 444bb61 && git push origin refs/tags/v1 --force` (blocked in-session by the tag force-push guard, needs the owner to run it or approve it). An immutable `v2.0.0` tag was attempted and errored on creation; the moving `v2` tag stands in its place.
...
- [x] **All three Milestone 6 live-verification boxes closed on stable infrastructure** — 2026-08-28 via throwaway PR #12, independently re-verified the same day via PRs #14 (drift → resolve) and #15 (noise skip). The repo default `gemini-3.7-flash` failed every call on this API key (instant error → neutral degrade, no comment); the Render service was pointed at `gemini-2.5-flash` and the in-repo default has now been changed to match — so the follow-up is **resolved**.
...
| **Default LLM** | Gemini 2.5 Flash | High speed, cost-effective structured JSON evaluation |
...
- **Gemini model default:** ~~the repo ships `GEMINI_MODEL` defaulting to `gemini-3.7-flash`, which failed every call on the deployed key.~~ Resolved 2026-08-28 — the in-repo default is now `gemini-2.5-flash` across `app/config.py`, `app/services/llm/gemini.py`, `README.md`, `GUIDE.md`, `.env.example`, and the tests; it matches what the Render service runs.
...
- **2026-08-28 (latest):** Independently re-ran the full Milestone 6 round-trip against the live Render deployment. PR #14: an undocumented `DIFF_CONTEXT_LINES` setting → `readme-realist[bot]` drift comment (exact README row suggested) + `neutral` Check Run; a follow-up commit documenting it rewrote the *same* comment to "up to date" and turned the check `success`. PR #15 (one-blank-line diff): `success` / "No documentation impact", no comment, no LLM call. Both PRs closed unmerged, branches deleted. Then **fixed the stale default Gemini model** repo-wide: `gemini-3.7-flash` → `gemini-2.5-flash` in `app/config.py`, `app/services/llm/gemini.py`, `README.md`, `GUIDE.md`, `.env.example`, and the affected tests — now matching what Render runs. Suite **330/330**, `ruff` + `ruff format` + `mypy` clean. The `v1`-alias-forward step is prepared but its tag force-push is left for the owner to run.
- **2026-08-28 (earlier):** Closed **all three Milestone 6 live-verification boxes** against the Render deployment via throwaway PR #12: (1) an undocumented `DIFF_CONTEXT_LINES` setting drew a `NEEDS_UPDATE` comment with the exact README row + advisory Check Run; (2) documenting it rewrote the *same* comment to up-to-date and turned the Check Run green; (3) a whitespace-only push produced no comment churn and a green check. PR #12 closed unmerged, branch deleted. **Root cause found along the way:** the repo default `GEMINI_MODEL=gemini-3.7-flash` failed every call on the deployed API key (instant error → neutral degrade, no comment); switching Render to `GEMINI_MODEL=gemini-2.5-flash` fixed it immediately. Milestone 6 is now **done**. Flagged a follow-up to change the repo's default Gemini model in `app/config.py`, `app/services/llm/gemini.py`, tests, `README.md`, `GUIDE.md`, and `.env.example`.
